...
#
# V1.7 adds live annotation-history and overlap awareness:
#   * current events show how many times and under which chapters they
#     have already been annotated;
#   * proposed chapter ranges summarize reused and unannotated events;
#   * near-duplicate chapters trigger a review warning but remain allowed.
# It retains the thematic-compilation editor, timestamp normalization,
# and range-enabled master MP4.
...
normalize_chapter_types <- function(data) {
  ...
  for (column_name in missing_columns) {
    ...
    if (is.logical(template_column)) {
      data[[column_name]] <- rep(NA, nrow(data))
    } else if (is.integer(template_column)) {
      data[[column_name]] <- rep(NA_integer_, nrow(data))
    } else if (is.numeric(template_column)) {
      data[[column_name]] <- rep(NA_real_, nrow(data))
    } else {
      data[[column_name]] <- rep(NA_character_, nrow(data))
    }
  }

  # readr may auto-parse timestamp-looking text as POSIXct on later runs.
  # Coerce every field back to the stable schema used by the editor before
  # rows are combined or written.
...
load_chapters <- function() {
  ...
  normalize_chapter_types(loaded)
}
...
save_chapter_system <- function(chapters) {
  chapters <- normalize_chapter_types(chapters) %>%
    arrange(chapter_order, start_seconds) %>%
...
append_export_manifest <- function(new_row) {
  ...
  # Keep timestamp fields as plain text so later app sessions cannot produce
  # POSIXct-versus-character bind_rows() conflicts.
  if ("export_created_at" %in% names(old)) {
    old$export_created_at <- as.character(old$export_created_at)
  }

  if ("export_created_at" %in% names(new_row)) {
    new_row$export_created_at <- as.character(
      new_row$export_created_at
    )
  }
...
(function () {
  ...
  function bindThematicPreviewEvents() {
    var preview = byId('thematic_preview_player');

    if (!preview || preview.dataset.previewEventsBound === 'true') {
      return;
    }

    preview.dataset.previewEventsBound = 'true';

    preview.addEventListener('timeupdate', function () {
      if (
        Number.isFinite(thematicPreviewEnd) &&
        preview.currentTime >= thematicPreviewEnd
      ) {
        preview.pause();
        preview.currentTime = thematicPreviewEnd;
      }
    ...
    preview.addEventListener('ended', function () {
      thematicPreviewEnd = null;
    });
  }

  function previewThematicChapter(startSeconds, endSeconds) {
    bindThematicPreviewEvents();
    ...
    tags$style(
      HTML(
        "
          ...
          .thematic-include-checkbox {
            width: 18px;
            height: 18px;
            cursor: pointer;
          }
          .thematic-toolbar .btn {
            margin-right: 5px;
            margin-bottom: 6px;
          }
          .thematic-preview-player {
            width: 100%;
            max-height: 430px;
            background: black;
          }
          ...
          .annotation-status-once {
            background: #e8f2fb;
            border-left-color: #337ab7;
            color: #24587f;
          }
          .annotation-status-multiple {
            background: #fff0dc;
            border-left-color: #d98213;
            color: #7a4a06;
          }
          .proposed-overlap-summary {
            padding: 9px 11px;
            margin-bottom: 9px;
            background: #eef4f8;
            border-left: 5px solid #4a789c;
          }
          .duplicate-warning {
            background: #fff0dc;
            border-left: 5px solid #d98213;
            padding: 9px 11px;
            margin-top: 8px;
          }
        "
  ...
  navbarPage(
    ...
    fluidRow(
      column(
        ...
        div(
          class = "movie-panel",
          h4("Current event annotation history"),
          uiOutput("current_event_annotation_status"),
          DTOutput("current_event_annotation_table"),
          br(),
          actionButton(
            "seek_current_event_annotation",
            "Seek to selected annotation",
            class = "btn-info"
          )
        ),
        div(
      ...
      column(
        ...
        div(
          class = "movie-panel",
          h4("Proposed chapter overlap"),
          uiOutput("proposed_chapter_overlap_summary"),
          DTOutput("proposed_chapter_overlap_table"),
          br(),
          actionButton(
            "seek_proposed_overlap_chapter",
            "Seek to selected overlapping chapter",
            class = "btn-info"
          )
        )
      )
  ...
  tabPanel(
    ...
    fluidRow(
      column(
        ...
        div(
          class = "movie-panel",
          h4("Compilation chapters"),
          p(
            "Tick the chapters to include, select a row to preview it, and ",
            "move the selected chapter up or down to set the compilation order."
          ),
          div(
            class = "thematic-toolbar",
            actionButton(
              "thematic_move_up",
              "Move selected up",
              class = "btn-default"
            ...
            actionButton(
              "thematic_preview_selected",
              "Preview selected",
              class = "btn-info"
            ),
            actionButton(
              "thematic_stop_preview",
              "Stop preview",
              class = "btn-default"
            ),
            actionButton(
              "thematic_include_all",
              "Include all",
              class = "btn-success"
            ),
            actionButton(
              "thematic_exclude_all",
              "Exclude all",
              class = "btn-warning"
            )
          ),
          DTOutput("thematic_chapter_table"),
          uiOutput("thematic_compilation_summary")
        )
      ),
      column(
        width = 5,
        div(
          class = "movie-panel",
          h4("Selected chapter preview"),
          uiOutput("thematic_selected_chapter_summary"),
          uiOutput("thematic_preview_ui")
        ),
        div(
          class = "movie-panel",
          ...
          p(
            "Only checked chapters are compiled, in the order shown in the table."
          ),
          actionButton(
...
server <- function(input, output, session) {
  ...
  pending_duplicate_chapter <- reactiveVal(NULL)
  ...
  selected_thematic_chapter_row <- reactive({
    selected <- input$thematic_chapter_table_rows_selected
  ...
  chapter_event_index <- reactive({
    ...
    rows <- vector("list", nrow(current))

    for (i in seq_len(nrow(current))) {
      chapter <- current[i, , drop = FALSE]

      rows[[i]] <- events_overlapping(
        chapter$start_seconds,
        chapter$end_seconds
      ) %>%
        transmute(
    ...
    bind_rows(rows) %>%
      distinct(chapter_id, event_id, .keep_all = TRUE)
  })

  current_event_annotations <- reactive({
    ...
    if (
      ...
      return(chapters()[0, , drop = FALSE])
    }

    linked_ids <- chapter_event_index() %>%
      filter(event_id == item$event_id) %>%
      distinct(chapter_id) %>%
      pull(chapter_id)

    chapters() %>%
      filter(chapter_id %in% linked_ids) %>%
      arrange(chapter_order, start_seconds)
  })

  proposed_chapter_overlap <- reactive({
    if (
    ...
    proposed_events <- events_overlapping(
      markers$start,
      markers$end
    )

    proposed_duration <- markers$end - markers$start
    current_index <- chapter_event_index()
    current_chapters <- chapters()

    if (nrow(proposed_events) == 0) {
      return(
        list(
          proposed_event_count = 0L,
          annotated_event_count = 0L,
          unannotated_event_count = 0L,
          overlapping_chapter_count = 0L,
          maximum_event_reuse = 0L,
          overlap_chapters = tibble()
        )
      )
    }

    proposed_event_ids <- unique(proposed_events$event_id)

    usage <- current_index %>%
      filter(event_id %in% proposed_event_ids) %>%
      count(event_id, name = "annotation_count")

    annotated_event_count <- length(
      intersect(proposed_event_ids, usage$event_id)
    )

    overlap_chapters <- current_index %>%
      filter(event_id %in% proposed_event_ids) %>%
      group_by(chapter_id) %>%
      summarise(
        shared_event_count = n_distinct(event_id),
        .groups = "drop"
      ) %>%
      left_join(
        current_chapters %>%
      ...
      mutate(
        event_overlap_fraction_proposed = ifelse(
          length(proposed_event_ids) > 0,
          shared_event_count / length(proposed_event_ids),
          0
        ),
        time_overlap_seconds = pmax(
          0,
          pmin(end_seconds, markers$end) -
            pmax(start_seconds, markers$start)
        ),
        time_overlap_fraction_proposed = ifelse(
          proposed_duration > 0,
          time_overlap_seconds / proposed_duration,
          0
        ),
        time_overlap_fraction_existing = ifelse(
          duration_seconds > 0,
          time_overlap_seconds / duration_seconds,
          0
        ),
        near_duplicate =
          event_overlap_fraction_proposed >= 0.80 |
          time_overlap_fraction_proposed >= 0.80
      ) %>%
      arrange(
        desc(near_duplicate),
        desc(time_overlap_fraction_proposed),
        desc(event_overlap_fraction_proposed),
        chapter_order
      )

    list(
      proposed_event_count = length(proposed_event_ids),
      annotated_event_count = annotated_event_count,
      unannotated_event_count =
        length(proposed_event_ids) - annotated_event_count,
      overlapping_chapter_count = nrow(overlap_chapters),
      maximum_event_reuse = if (nrow(usage) > 0) {
        max(usage$annotation_count, na.rm = TRUE)
      } else {
        0L
      },
  ...
  output$current_event_annotation_status <- renderUI({
    item <- current_timeline_item()
    ...
    if (
      ...
      return(
        div(
          class = "annotation-status-box annotation-status-none",
          "Move the movie cursor onto an event."
        )
      )
    }

    annotations <- current_event_annotations()
    count <- nrow(annotations)

    if (count == 0) {
      div(
        class = "annotation-status-box annotation-status-none",
        strong("NOT YET ANNOTATED"),
        br(),
        paste("Event", item$event_id, "has not been used in a chapter.")
      )
    } else if (count == 1) {
      div(
        class = "annotation-status-box annotation-status-once",
        strong("ANNOTATED ONCE"),
        br(),
        paste("Event", item$event_id, "appears in one chapter.")
      )
    } else {
      div(
        class = "annotation-status-box annotation-status-multiple",
        strong(paste("ANNOTATED", count, "TIMES")),
        br(),
        paste("Event", item$event_id, "appears in multiple chapters.")
      )
    }
  })

  output$current_event_annotation_table <- renderDT({
    display <- current_event_annotations() %>%
      transmute(
        ID = chapter_id,
  ...
  observeEvent(input$seek_current_event_annotation, {
    selected <- input$current_event_annotation_table_rows_selected
    available <- current_event_annotations()

    if (
      length(selected) != 1 ||
      selected < 1 ||
      selected > nrow(available)
    ) {
      showNotification(
        "Select an annotation row first.",
        type = "error"
      )
      return()
    }

    chapter <- available[selected, , drop = FALSE]

    session$sendCustomMessage(
      "seekMainMovie",
      list(
        time = as.numeric(chapter$start_seconds),
        play = FALSE
      )
    )
  })
  ...
  output$proposed_chapter_overlap_summary <- renderUI({
    overlap <- proposed_chapter_overlap()

    if (is.null(overlap)) {
      return(
        div(
          class = "proposed-overlap-summary",
          "Mark both a valid chapter start and end to inspect overlap."
        )
      )
    }

    warning_rows <- overlap$overlap_chapters %>%
      filter(near_duplicate %in% TRUE)
    ...
    tagList(
      div(
        ...
        strong(
          paste0(
            overlap$proposed_event_count,
            " event(s) in proposed chapter"
          )
        ),
        br(),
        paste0(
          overlap$unannotated_event_count,
          " never annotated; ",
          overlap$annotated_event_count,
          " already annotated; ",
          overlap$overlapping_chapter_count,
          " overlapping chapter(s); maximum event reuse = ",
          overlap$maximum_event_reuse,
          "."
        )
      ),
      if (nrow(warning_rows) > 0) {
        div(
          ...
          strong("Possible near-duplicate chapter"),
          br(),
          paste0(
            nrow(warning_rows),
            " existing chapter(s) overlap at least 80% of the proposed ",
            "events or proposed duration. Reuse is allowed, but review them first."
          )
        )
      }
    )
  ...
  output$proposed_chapter_overlap_table <- renderDT({
    overlap <- proposed_chapter_overlap()

    if (
      is.null(overlap) ||
      nrow(overlap$overlap_chapters) == 0
    ) {
      display <- tibble(
        Chapter = character(),
        Theme = character(),
        Shared_events = integer(),
        Event_overlap_pct = numeric(),
        Time_overlap_s = numeric(),
        Time_overlap_pct = numeric(),
        Near_duplicate = logical()
      )
    } else {
      display <- overlap$overlap_chapters %>%
        transmute(
          Chapter = paste0(chapter_id, " — ", chapter_title),
          Theme = theme,
          Shared_events = shared_event_count,
          Event_overlap_pct = round(
            100 * event_overlap_fraction_proposed,
            1
          ),
          Time_overlap_s = round(time_overlap_seconds, 2),
          Time_overlap_pct = round(
            100 * time_overlap_fraction_proposed,
            1
          ),
  ...
  observeEvent(input$seek_proposed_overlap_chapter, {
    overlap <- proposed_chapter_overlap()
    selected <- input$proposed_chapter_overlap_table_rows_selected

    if (
      is.null(overlap) ||
      length(selected) != 1 ||
      selected < 1 ||
      selected > nrow(overlap$overlap_chapters)
    ) {
      showNotification(
        "Select an overlapping chapter first.",
        type = "error"
      )
      return()
    }

    chapter <- overlap$overlap_chapters[selected, , drop = FALSE]
  ...
  observeEvent(input$add_chapter, {
    ...
    near_duplicates <- if (
      ...
      overlap$overlap_chapters %>%
        filter(near_duplicate %in% TRUE)
    }

    if (nrow(near_duplicates) == 0) {
      create_new_chapter()
      return()
    }

    strongest <- near_duplicates %>%
      arrange(
        desc(time_overlap_fraction_proposed),
        desc(event_overlap_fraction_proposed)
      ) %>%
    ...
    pending_duplicate_chapter(strongest)

    showModal(
      modalDialog(
        title = "Possible near-duplicate chapter",
        tagList(
          p(
            "The proposed chapter substantially overlaps an existing chapter. ",
            "Repeated thematic annotation is allowed, but review this before adding."
          ),
          tags$ul(
            tags$li(
              strong("Existing chapter: "),
              paste0(
                strongest$chapter_id,
                " — ",
                strongest$chapter_title
              )
            ),
            tags$li(
              strong("Theme: "),
            ...
            tags$li(
              strong("Shared proposed events: "),
              paste0(
                round(
                  100 * strongest$event_overlap_fraction_proposed,
                  1
                ),
                "%"
              )
            ...
            tags$li(
              strong("Shared proposed duration: "),
              paste0(
                round(
                  100 * strongest$time_overlap_fraction_proposed,
                  1
                ),
                "%"
              )
  ...
  observeEvent(input$review_duplicate_chapter, {
    chapter <- pending_duplicate_chapter()

    if (is.null(chapter) || nrow(chapter) == 0) {
      removeModal()
      return()
    }
  ...
  observeEvent(input$confirm_add_duplicate, {
    removeModal()
    create_new_chapter()
    pending_duplicate_chapter(NULL)
  })
  ...
  output$thematic_chapter_table <- renderDT({
    ...
    include_checkbox <- vapply(
      seq_len(nrow(current)),
      function(i) {
        checked_text <- if (
          isTRUE(current$include_in_thematic_compilation[i])
        ) {
          "checked"
        } else {
          ""
        ...
        sprintf(
          paste0(
            '<input type="checkbox" ',
            'class="thematic-include-checkbox" ',
            'data-chapter-id="%s" %s>'
          ),
          htmltools::htmlEscape(
            current$chapter_id[i],
            attribute = TRUE
          ),
          checked_text
        )
    ...
    display <- current %>%
      transmute(
        Compile = include_checkbox,
        Order = chapter_order,
        Chapter = htmltools::htmlEscape(chapter_title),
        Theme = htmltools::htmlEscape(theme),
        Start = vapply(start_seconds, format_seconds, character(1)),
        End = vapply(end_seconds, format_seconds, character(1)),
        Duration_s = round(duration_seconds, 2),
        Sharing = htmltools::htmlEscape(sharing_status)
      )
    ...
    datatable(
      ...
      callback = JS(
        ...
        table.on(
          'click',
          'input.thematic-include-checkbox',
          function(event) {
        ...
        table.on(
          ...
          function(event) {
            ...
            Shiny.setInputValue(
              'thematic_include_change',
              {
                chapter_id: this.getAttribute('data-chapter-id'),
  ...
  output$thematic_preview_ui <- renderUI({
    tags$video(
      id = "thematic_preview_player",
      src = range_enabled_video_url,
      type = "video/mp4",
      controls = NA,
      preload = "metadata",
      playsinline = NA,
      class = "thematic-preview-player"
    )
  })

  output$thematic_selected_chapter_summary <- renderUI({
    chapter <- selected_thematic_chapter_row()

    if (is.null(chapter)) {
      return(
        p("Select a chapter row to preview it.")
      )
    }

    div(
      class = "thematic-summary",
      strong(chapter$chapter_title),
      br(),
      paste0(
        "Order ",
        chapter$chapter_order,
        " · ",
        format_seconds(chapter$start_seconds),
        "–",
        format_seconds(chapter$end_seconds),
        " · ",
        round(chapter$duration_seconds, 2),
        " s"
      ),
      br(),
      paste0(
        "Included: ",
        ifelse(
          isTRUE(chapter$include_in_thematic_compilation),
          "Yes",
          "No"
        )
      )
  ...
  output$thematic_compilation_summary <- renderUI({
    current <- chapters() %>%
    ...
    div(
      ...
      paste0(
        "Compiled duration: ",
        round(
          sum(included$duration_seconds, na.rm = TRUE),
          2
        ),
        " seconds"
      )
  ...
  observeEvent(input$thematic_include_change, {
    change <- input$thematic_include_change

    if (
      is.null(change$chapter_id) ||
      !nzchar(change$chapter_id)
    ) {
      return()
    }

    current <- chapters()
    row_index <- match(change$chapter_id, current$chapter_id)

    if (is.na(row_index)) {
      return()
    }

    current$include_in_thematic_compilation[row_index] <-
      isTRUE(change$checked)

    current <- save_chapter_system(current)
    chapters(current)
  })

  move_selected_thematic_chapter <- function(direction) {
    selected <- selected_thematic_chapter_row()

    if (is.null(selected)) {
      showNotification(
        "Select a chapter in the compilation table first.",
        type = "error"
      )
      return()
    }
  ...
  observeEvent(input$thematic_move_up, {
    move_selected_thematic_chapter(-1L)
  })

  observeEvent(input$thematic_move_down, {
    move_selected_thematic_chapter(1L)
  })

  observeEvent(input$thematic_include_all, {
    current <- chapters()
    current$include_in_thematic_compilation <- TRUE
    current <- save_chapter_system(current)
    chapters(current)
  })

  observeEvent(input$thematic_exclude_all, {
    current <- chapters()
    current$include_in_thematic_compilation <- FALSE
    current <- save_chapter_system(current)
    chapters(current)
  })

  observeEvent(input$thematic_preview_selected, {
    chapter <- selected_thematic_chapter_row()
    ...
    session$sendCustomMessage(
      "previewThematicChapter",
      list(
        start = as.numeric(chapter$start_seconds),
        end = as.numeric(chapter$end_seconds)
      )
    )
  })

  observeEvent(input$thematic_stop_preview, {
    session$sendCustomMessage(
      "stopThematicPreview",
      list()
    )
