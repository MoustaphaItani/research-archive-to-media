...
if (nrow(missing_media) > 0) {
  readr::write_csv(
    missing_media,
    file.path(log_directory, "missing_source_media.csv")
  )

  stop(
    nrow(missing_media),
    " source-media file(s) are missing. See logs/missing_source_media.csv."
  )
}
...
render_event <- function(event_row, members) {
  ...
  clip_file <- file.path(event_clip_directory, paste0(event_id, ".mp4"))
