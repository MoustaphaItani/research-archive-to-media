...
# It writes only to outputs/_movie_inventory_v2_1.
# ============================================================
...
# Read the previous event membership BEFORE outputs are replaced. This registry
# lets unchanged or expanded events retain their existing IDs across archive
# rebuilds, even when new media are inserted earlier in the chronology.
previous_event_members_file <- file.path(
  output_directory,
  "06_event_members_v2_1.csv"
)

previous_event_members <- if (file.exists(previous_event_members_file)) {
  readr::read_csv(
    previous_event_members_file,
    show_col_types = FALSE,
    na = c("", "NA")
  ) %>%
    select(any_of(c("event_id", "relative_path"))) %>%
    filter(
      !is.na(event_id),
      nzchar(event_id),
      !is.na(relative_path),
    ...
    distinct(event_id, relative_path)
} else {
  tibble(
    event_id = character(),
...
# Exclude every folder generated by this media pipeline.
#
# These folders may contain pilot movies, rendered event clips, social
# exports, temporary working frames, editor previews, and archived builds.
# They are outputs derived from the source archive and must never be
# inventoried again as if they were original field media.
generated_directory_pattern <- paste0(
  "(^|/)",
  "(",
  "_movie_inventory[^/]*",
  "|_movie_pilot_[^/]*",
...
# Defensive validation: no generated pipeline output may enter the source
# inventory. Stop immediately if a future folder name bypasses the filter.
generated_rows_in_inventory <- grepl(
  generated_directory_pattern,
  paste0("/", gsub("\\\\", "/", file_inventory$relative_path)),
  ignore.case = TRUE
)

if (any(generated_rows_in_inventory)) {
  contaminated_rows <- file_inventory %>%
    filter(generated_rows_in_inventory) %>%
    select(relative_path, media_type, extension)

  readr::write_csv(
    contaminated_rows,
    file.path(output_directory, "ERROR_generated_outputs_in_inventory.csv"),
    na = ""
  )

  stop(
    sum(generated_rows_in_inventory),
    " generated pipeline output(s) entered the source inventory. ",
    "See ERROR_generated_outputs_in_inventory.csv."
  )
}
...
# Build a canonical signature for every newly detected event. The signature
# changes when event membership or source files change, while the persistent
# event ID is reused whenever the new event overlaps an event from the previous
# inventory.
event_identity <- inventory %>%
  arrange(event_key, relative_path) %>%
  group_by(event_key) %>%
  summarise(
    event_capture_time = min(effective_capture_time, na.rm = TRUE),
    first_inventory_order = min(inventory_order),
    member_count = n(),
    identity_seed = paste(sort(relative_path), collapse = "|"),
    source_signature_seed = paste(
      relative_path,
      file_size_bytes,
      as.numeric(file_modified_time),
      sep = "::",
      collapse = "|"
    ),
    .groups = "drop"
  ...
  mutate(
    ...
    event_source_signature = vapply(
      source_signature_seed,
      digest::digest,
      character(1),
      algo = "xxhash64",
      serialize = FALSE
...
new_members_for_matching <- inventory %>%
  select(event_key, relative_path) %>%
  distinct()

previous_event_sizes <- previous_event_members %>%
  count(event_id, name = "previous_member_count")

new_event_sizes <- new_members_for_matching %>%
  count(event_key, name = "new_member_count")

overlap_candidates <- new_members_for_matching %>%
  inner_join(previous_event_members, by = "relative_path") %>%
  count(event_key, event_id, name = "shared_member_count") %>%
  left_join(new_event_sizes, by = "event_key") %>%
  left_join(previous_event_sizes, by = "event_id") %>%
  mutate(
    overlap_new = shared_member_count / new_member_count,
    overlap_previous = shared_member_count / previous_member_count,
    overlap_score = pmax(overlap_new, overlap_previous)
  ) %>%
  arrange(
    desc(shared_member_count),
    desc(overlap_score),
    event_key,
    event_id
  )

# Greedy one-to-one assignment prevents one historical event ID from being
# attached to multiple newly split events.
matched_event_ids <- tibble(
  event_key = character(),
  previous_event_id = character(),
  shared_member_count = integer(),
  overlap_new = numeric(),
  overlap_previous = numeric()
)

used_new_keys <- character()
used_previous_ids <- character()

if (nrow(overlap_candidates) > 0) {
  for (i in seq_len(nrow(overlap_candidates))) {
    candidate <- overlap_candidates[i, , drop = FALSE]

    if (
      candidate$event_key %in% used_new_keys ||
      candidate$event_id %in% used_previous_ids
    ) {
      next
    }

    # Any identical source path is strong evidence of continuity. Requiring
    # at least half of either the old or new event protects against accidental
    # matches if event grouping changes substantially.
    acceptable_match <- candidate$shared_member_count >= 1 &&
      (
        candidate$overlap_new >= 0.5 ||
        candidate$overlap_previous >= 0.5
      )
    ...
    used_new_keys <- c(used_new_keys, candidate$event_key)
    used_previous_ids <- c(used_previous_ids, candidate$event_id)
  }
}

event_id_lookup <- event_identity %>%
  left_join(matched_event_ids, by = "event_key") %>%
  mutate(
    event_id = ifelse(
      !is.na(previous_event_id),
      previous_event_id,
      deterministic_event_id
    ),
    event_id_status = ifelse(
      !is.na(previous_event_id),
      "REUSED_PREVIOUS_ID_BY_SOURCE_OVERLAP",
      "NEW_DETERMINISTIC_ID"
    )
  ) %>%
  arrange(event_capture_time, first_inventory_order, event_key)

if (anyDuplicated(event_id_lookup$event_id)) {
  stop(
    "Expansion-safe event-ID assignment produced duplicate IDs. ",
...
event_plan <- inventory %>%
  ...
  summarise(
    ...
    member_count = n(),
    event_source_signature = dplyr::first(event_source_signature),
    event_id_status = dplyr::first(event_id_status),
    previous_event_id = dplyr::first(previous_event_id),
    shared_member_count = dplyr::first(shared_member_count),
    overlap_new = dplyr::first(overlap_new),
    overlap_previous = dplyr::first(overlap_previous),
...
event_registry_file <- file.path(output_directory, "10_event_id_registry_v3.csv")
...
summary_lines <- c(
  ...
  paste0(
    "Events retaining a previous ID: ",
    sum(event_plan$event_id_status == "REUSED_PREVIOUS_ID_BY_SOURCE_OVERLAP")
  ),
  paste0(
    "New deterministic event IDs: ",
    sum(event_plan$event_id_status == "NEW_DETERMINISTIC_ID")
  ),
  paste0(
    "Single-photo events: ",
